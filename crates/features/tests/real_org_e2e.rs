...
async fn e2e_api_version_detected() {
    ...
    assert!(v.contains('.'), "api version should look like NN.0, got {v:?}");
...
async fn e2e_soql_completion_real() {
    ...
    // SELECT position → real fields + a function + the FROM keyword.
    let select = complete_fields(&inv, &root, &org(), "SELECT  FROM Account", 7).await;
    let l = labels(&select);
    assert!(l.iter().any(|x| x == "Name"), "real field Name missing: {l:?}");
    ...
    assert!(
        select.iter().any(|c| c.kind == soql_lang::CandidateKind::Function),
        "expected at least one Function-kind candidate"
    );

    // FROM position → real object names filtered by the typed prefix.
    let from = complete_fields(&inv, &root, &org(), "SELECT Id FROM Acc", 18).await;
    let lf = labels(&from);
    assert!(lf.iter().any(|x| x == "Account"), "FROM should offer Account: {lf:?}");
    assert!(
        from.iter().any(|c| c.kind == soql_lang::CandidateKind::Object),
...
async fn e2e_soql_diagnostics_real() {
    ...
    assert!(good.is_empty(), "valid fields should not be flagged: {good:?}");
}
...
async fn e2e_run_soql_real() {
    let result = run_query(&invoker(), "SELECT Id, Name FROM Account LIMIT 5", Some(&org()))
        .await
    ...
    assert!(table.columns.iter().any(|c| c == "Name"), "Name column expected");
}
...
async fn e2e_dml_insert_query_delete() {
    ...
    // Insert.
    let ins = run_anon(&inv, &format!("insert new Account(Name='{marker}');"), Some(&o))
        .await
        .expect("insert call");
    assert!(ins.result.success, "insert failed: {:?}", ins.result.error());
    ...
    .await
    .expect("delete call");
    assert!(del.result.success, "delete failed: {:?}", del.result.error());
    ...
    assert_eq!(q1.total_size, 1, "inserted record should be found exactly once");
